...
module.exports = {
  ...
  checkFunctionCondi:checkFunctionCondi,
  callFunctionRes:callFunctionRes,
  checkFunctionCondiData:checkFunctionCondiData,
  GetNum:GetNum,
...
function checkFunctionCondi(funcPra,OPENID,FunctionName)
{
  if (IsNullOrEmpty(funcPra)||IsNullOrEmpty(funcPra.PMObject)) {
    errMsg = ""+FunctionName+":创建任务时，参数为空。"
    result = []
    return {
      errMsg,
      result
...
function checkFunctionCondiData(funcPra,OPENID,FunctionName,Data)
{
  if (IsNullOrEmpty(funcPra)||IsNullOrEmpty(Data)) {
    errMsg = ""+FunctionName+":创建任务时，参数为空。"
    result = []
    return {
      errMsg,
...
async function  callFunctionRes(cloud,DBExeData) {
  try {
    ...
    console.log(serRes)
    var TempData = getResFromReturn(serRes) //getResFromReturn整合和简化sertRes供前端使用。
    ...
    errMsg = GetStr(e) //getResFromReturn整合和简化sertRes供前端使用。
    result = []
    return {
      errMsg,
      result
...
function getOneControlFromList(selectedID, TempList,field) {
